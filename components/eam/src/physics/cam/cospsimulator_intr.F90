module cospsimulator_intr
  ...
  use mod_cosp_config,      only: pres_binCenters, pres_binEdges, tau_binCenters,      &
       ...
       CFODD_NCLASS, WR_NREGIME, NOBSTYPE, SLWC_NCOT, SLWC_HISTCOT, COT_NCLASS,        &
...
!PMA
      ...
      ! float clmodis_lwpre ( time, plev, tau, loc )
       call addfld ('CLMODIS_LWPR_MaCST',(/'cosp_lwp_modis','cosp_reffliq  '/),'A','%','MODIS Cloud Area Fraction (LWP-RE histogram) - MODIS/CloudSat', &
            flag_xyfill=.true., fill_value=R_UNDEF)
       call addfld ('CLMODIS_LWPR_MaCAL',(/'cosp_lwp_modis','cosp_reffliq  '/),'A','%','MODIS Cloud Area Fraction (LWP-RE histogram) - MODIS/CALIPSO', &
            flag_xyfill=.true., fill_value=R_UNDEF)
       call addfld ('CLMODIS_LWPR_MoCAL',(/'cosp_lwp_modis','cosp_reffliq  '/),'A','%','MODIS Cloud Area Fraction (LWP-RE histogram) - MODIS or CALIPSO', &
            flag_xyfill=.true., fill_value=R_UNDEF)
       ...
       ! CMB 04/19/24
       call add_default ('CLMODIS_LWPR_MaCST', cosp_histfile_num, ' ')
       call add_default ('CLMODIS_LWPR_MaCAL', cosp_histfile_num, ' ')
       call add_default ('CLMODIS_LWPR_MoCAL', cosp_histfile_num, ' ')
    ...
    if ((lradar_sim) .and. (lmodis_sim) .and. (llidar_sim)) then
       ...
       call addfld ('CFODD_NTOTAL10_CS',(/'cosp_cfodd_dbze','cosp_cfodd_icod' /),&
              'A','1','# of CFODD MODIS-CALIPSO, small reff', flag_xyfill=.true.,&
              fill_value=R_UNDEF)
       call addfld ('CFODD_NTOTAL11_CS',(/'cosp_cfodd_dbze','cosp_cfodd_icod' /),&
              'A','1','# of CFODD MODIS-CALIPSO, med reff', flag_xyfill=.true.,&
              fill_value=R_UNDEF)
       call addfld ('CFODD_NTOTAL12_CS',(/'cosp_cfodd_dbze','cosp_cfodd_icod' /),&
              'A','1','# of CFODD MODIS-CALIPSO, large reff', flag_xyfill=.true.,&
              fill_value=R_UNDEF)
       call addfld ('CFODD_NTOTAL13_CS',(/'cosp_cfodd_dbze','cosp_cfodd_icod' /),&
              'A','1','# of CFODD MODIS-CALIPSO, max dBZ<20, 4<=cot<20, small reff', flag_xyfill=.true.,&
              fill_value=R_UNDEF)
       call addfld ('CFODD_NTOTAL14_CS',(/'cosp_cfodd_dbze','cosp_cfodd_icod' /),&
              'A','1','# of CFODD MODIS-CALIPSO, max dBZ<20, 4<=cot<20, med reff', flag_xyfill=.true.,&
              fill_value=R_UNDEF)
       call addfld ('CFODD_NTOTAL15_CS',(/'cosp_cfodd_dbze','cosp_cfodd_icod' /),&
              'A','1','# of CFODD MODIS-CALIPSO, max dBZ<20, 4<=cot<20, large reff', flag_xyfill=.true.,&
              fill_value=R_UNDEF)	      
            
        call addfld ('SLWC_COT1',(/'cosp_slwc_cot' /),&
             'A','1','# of SLWCs (05 <= Reff < 12 micron) binned by MODIS cloud optical thickness',&
             flag_xyfill=.true., fill_value=R_UNDEF)
       ...
       call addfld ('SLWC_COT10',(/'cosp_slwc_cot' /), 'A','1',&
              ...
              flag_xyfill=.true.,fill_value=R_UNDEF)
       call addfld ('SLWC_COT11',(/'cosp_slwc_cot' /), 'A','1',&
              '# of SLWCs (12 <= Reff < 18 micron) detected by CALIPSO/MODIS binned by MODIS cloud optical thickness',&
              flag_xyfill=.true.,fill_value=R_UNDEF)
       call addfld ('SLWC_COT12',(/'cosp_slwc_cot' /), 'A','1',&
              '# of SLWCs (18 <= Reff < 30 micron) detected by CALIPSO/MODIS binned by MODIS cloud optical thickness',&
              flag_xyfill=.true.,fill_value=R_UNDEF)
...
!        call addfld('CFODD_NICOD',(/'cosp_cfodd_nicod'/),'A', '1', &
	...
	call add_default ('CFODD_NTOTAL11_CS',cosp_histfile_num, ' ')
	call add_default ('CFODD_NTOTAL12_CS',cosp_histfile_num, ' ')
	call add_default ('CFODD_NTOTAL13_CS',cosp_histfile_num, ' ')
	call add_default ('CFODD_NTOTAL14_CS',cosp_histfile_num, ' ')
	call add_default ('CFODD_NTOTAL15_CS',cosp_histfile_num, ' ')
...
!    integer, parameter :: nf_modis=20                    ! number of modis outputs
    integer, parameter :: nf_modis=49                    ! number of modis outputs ! YQIN
...
!                         'CLDZOPQ_CAL_SE' /)
    ...
    character(len=max_fieldname_len),dimension(nf_modis) :: &
         fname_modis=(/'CLTMODIS      ','CLWMODIS      ','CLIMODIS      ','CLHMODIS      ','CLMMODIS      ',&
                       ...
                       'CLMODIS_LWPR_MaCST','CLMODIS_LWPR_MaCAL',          & ! CMB 04/19/24
                       'CLMODIS_LWPR_MoCAL'                                & ! CMB 04/19/24
                       /)
...
!    real(r8) :: opacity_cal_2d(pcols,nht_cosp) 
    ...
    ! CMB 04/19/24
    real(r8) :: clmodis_lwpre_cam_macst(pcols,nlwp_cosp_modis*numMODISReffLiqBins)
    real(r8) :: clmodis_lwpre_macst(pcols,nlwp_cosp_modis,numMODISReffLiqBins)
    real(r8) :: clmodis_lwpre_cam_macal(pcols,nlwp_cosp_modis*numMODISReffLiqBins)
    real(r8) :: clmodis_lwpre_macal(pcols,nlwp_cosp_modis,numMODISReffLiqBins)
    real(r8) :: clmodis_lwpre_cam_mocal(pcols,nlwp_cosp_modis*numMODISReffLiqBins)
    real(r8) :: clmodis_lwpre_mocal(pcols,nlwp_cosp_modis,numMODISReffLiqBins)
    ...
    real(r8) :: cfodd_ntotal11(pcols,CFODD_NDBZE,CFODD_NICOD)
    real(r8) :: cfodd_ntotal12(pcols,CFODD_NDBZE,CFODD_NICOD)
    real(r8) :: cfodd_ntotal13(pcols,CFODD_NDBZE,CFODD_NICOD)
    real(r8) :: cfodd_ntotal14(pcols,CFODD_NDBZE,CFODD_NICOD)
    real(r8) :: cfodd_ntotal15(pcols,CFODD_NDBZE,CFODD_NICOD)
    ...
    real(r8) :: cfodd_ntotal11_cs(pcols,CFODD_NDBZE*CFODD_NICOD)
    real(r8) :: cfodd_ntotal12_cs(pcols,CFODD_NDBZE*CFODD_NICOD)
    real(r8) :: cfodd_ntotal13_cs(pcols,CFODD_NDBZE*CFODD_NICOD)
    real(r8) :: cfodd_ntotal14_cs(pcols,CFODD_NDBZE*CFODD_NICOD)
    real(r8) :: cfodd_ntotal15_cs(pcols,CFODD_NDBZE*CFODD_NICOD)
    ...
    real(r8) :: slwc_cot11(pcols,SLWC_NCOT)
    real(r8) :: slwc_cot12(pcols,SLWC_NCOT)
    ...
    cfodd_ntotal11(1:pcols,1:CFODD_NDBZE,1:CFODD_NICOD) = R_UNDEF
    cfodd_ntotal12(1:pcols,1:CFODD_NDBZE,1:CFODD_NICOD) = R_UNDEF
    cfodd_ntotal13(1:pcols,1:CFODD_NDBZE,1:CFODD_NICOD) = R_UNDEF
    cfodd_ntotal14(1:pcols,1:CFODD_NDBZE,1:CFODD_NICOD) = R_UNDEF
    cfodd_ntotal15(1:pcols,1:CFODD_NDBZE,1:CFODD_NICOD) = R_UNDEF
...
!    opacity_cal_2d(1:pcols,1:nht_cosp)               = R_UNDEF
    ...
    cfodd_ntotal11_cs(1:pcols,1:CFODD_NDBZE*CFODD_NICOD) = R_UNDEF
    cfodd_ntotal12_cs(1:pcols,1:CFODD_NDBZE*CFODD_NICOD) = R_UNDEF
    cfodd_ntotal13_cs(1:pcols,1:CFODD_NDBZE*CFODD_NICOD) = R_UNDEF
    cfodd_ntotal14_cs(1:pcols,1:CFODD_NDBZE*CFODD_NICOD) = R_UNDEF
    cfodd_ntotal15_cs(1:pcols,1:CFODD_NDBZE*CFODD_NICOD) = R_UNDEF
    ...
    ! CMB 04/19/24
    clmodis_lwpre_cam_macst(1:pcols,1:nlwp_cosp_modis*numMODISReffLiqBins) = R_UNDEF
    clmodis_lwpre_macst(1:pcols,1:nlwp_cosp_modis,1:numMODISReffLiqBins)   = R_UNDEF
    clmodis_lwpre_cam_macal(1:pcols,1:nlwp_cosp_modis*numMODISReffLiqBins) = R_UNDEF
    clmodis_lwpre_macal(1:pcols,1:nlwp_cosp_modis,1:numMODISReffLiqBins)   = R_UNDEF
    clmodis_lwpre_cam_mocal(1:pcols,1:nlwp_cosp_modis*numMODISReffLiqBins) = R_UNDEF
    clmodis_lwpre_mocal(1:pcols,1:nlwp_cosp_modis,1:numMODISReffLiqBins)   = R_UNDEF
    ! ######################################################################################
          ...
          do i=1,nlwp_cosp_modis
            do k=1,numMODISReffLiqBins
                where(cam_sunlit(1:ncol) .eq. 0)
                    ...
                    ! CMB 04/19/24
                    cospOUT%modis_LWP_vs_ReffLIQ_MaCST(1:ncol,i,k) = R_UNDEF
                    cospOUT%modis_LWP_vs_ReffLIQ_MaCAL(1:ncol,i,k) = R_UNDEF
                    cospOUT%modis_LWP_vs_ReffLIQ_MoCAL(1:ncol,i,k) = R_UNDEF
                end where
            enddo
          enddo
...
!       opacity_cal_2d(1:pcols,1:nht_cosp)  = cospOUT%calipso_lidarcldtype(:,:,4)
    ...
    if (lmodis_sim) then
       ...
       ! CMB 04/19/24
       clmodis_lwpre_macst(1:ncol,1:nlwp_cosp_modis,1:numMODISReffLiqBins) = cospOUT%modis_LWP_vs_ReffLIQ_MaCST
       clmodis_lwpre_macal(1:ncol,1:nlwp_cosp_modis,1:numMODISReffLiqBins) = cospOUT%modis_LWP_vs_ReffLIQ_MaCAL
       clmodis_lwpre_mocal(1:ncol,1:nlwp_cosp_modis,1:numMODISReffLiqBins) = cospOUT%modis_LWP_vs_ReffLIQ_MoCAL
    endif
	...
	cfodd_ntotal11(1:ncol,1:CFODD_NDBZE,1:CFODD_NICOD) = cospOUT%cfodd_ntotal(:,:,:,11)
	cfodd_ntotal12(1:ncol,1:CFODD_NDBZE,1:CFODD_NICOD) = cospOUT%cfodd_ntotal(:,:,:,12)
	cfodd_ntotal13(1:ncol,1:CFODD_NDBZE,1:CFODD_NICOD) = cospOUT%cfodd_ntotal(:,:,:,13)
	cfodd_ntotal14(1:ncol,1:CFODD_NDBZE,1:CFODD_NICOD) = cospOUT%cfodd_ntotal(:,:,:,14)
	cfodd_ntotal15(1:ncol,1:CFODD_NDBZE,1:CFODD_NICOD) = cospOUT%cfodd_ntotal(:,:,:,15)
        ...
        slwc_cot11(1:ncol,1:SLWC_NCOT) = cospOUT%slwccot(:,:,11)
        slwc_cot12(1:ncol,1:SLWC_NCOT) = cospOUT%slwccot(:,:,12)
    endif
    ...
    do i=1,ncol
       ...
       if ((lradar_sim) .and. (lmodis_sim) .and. (llidar_sim)) then
		   ...
		   cfodd_ntotal11_cs(i,idic) = cfodd_ntotal11(i,id,ih)
		   cfodd_ntotal12_cs(i,idic) = cfodd_ntotal12(i,id,ih)
		   cfodd_ntotal13_cs(i,idic) = cfodd_ntotal13(i,id,ih)
		   cfodd_ntotal14_cs(i,idic) = cfodd_ntotal14(i,id,ih)
		   cfodd_ntotal15_cs(i,idic) = cfodd_ntotal15(i,id,ih)
                end do
            end do
       endif
       ...
       if (lmodis_sim) then
          ...
          do ip=1,numMODISReffLiqBins
             do it=1,nlwp_cosp_modis
                ...
                clmodis_lwpre_cam_macst(i,ipt) = clmodis_lwpre_macst(i,it,ip)
                clmodis_lwpre_cam_macal(i,ipt) = clmodis_lwpre_macal(i,it,ip)
                clmodis_lwpre_cam_mocal(i,ipt) = clmodis_lwpre_mocal(i,it,ip)
             end do
          end do
    ...
    if (lmodis_sim) then
    do i=1,ncol
      if (cltmodis_ALL(i).ge.0._r8 .and. cltmodis_ALL(i) .lt. 100._r8) then !.and.aerindex_in(i) .ge. 0.01_r8) then
...
!       call outfld('OPACITY_CAL_2D',  opacity_cal_2d,    pcols, lchnk)
    ...
    if ((lradar_sim) .and. (lmodis_sim) .and. (llidar_sim)) then
         ...
         where (cfodd_ntotal11_cs(:ncol,:CFODD_NDBZE*CFODD_NICOD) .eq. R_UNDEF)
             cfodd_ntotal11_cs(:ncol,:CFODD_NDBZE*CFODD_NICOD) = 0.0_r8
         end where

         where (cfodd_ntotal12_cs(:ncol,:CFODD_NDBZE*CFODD_NICOD) .eq. R_UNDEF)
             cfodd_ntotal12_cs(:ncol,:CFODD_NDBZE*CFODD_NICOD) = 0.0_r8
         end where

         where (cfodd_ntotal13_cs(:ncol,:CFODD_NDBZE*CFODD_NICOD) .eq. R_UNDEF)
             cfodd_ntotal13_cs(:ncol,:CFODD_NDBZE*CFODD_NICOD) = 0.0_r8
         end where

	 where (cfodd_ntotal14_cs(:ncol,:CFODD_NDBZE*CFODD_NICOD) .eq. R_UNDEF)
             cfodd_ntotal14_cs(:ncol,:CFODD_NDBZE*CFODD_NICOD) = 0.0_r8
         end where

	 where (cfodd_ntotal15_cs(:ncol,:CFODD_NDBZE*CFODD_NICOD) .eq. R_UNDEF)
             cfodd_ntotal15_cs(:ncol,:CFODD_NDBZE*CFODD_NICOD) = 0.0_r8
         end where
         ...
         call outfld('CFODD_NTOTAL11_CS', cfodd_ntotal11_cs, pcols, lchnk)
         call outfld('CFODD_NTOTAL12_CS', cfodd_ntotal12_cs, pcols, lchnk)
         call outfld('CFODD_NTOTAL13_CS', cfodd_ntotal13_cs, pcols, lchnk)
         call outfld('CFODD_NTOTAL14_CS', cfodd_ntotal14_cs, pcols, lchnk)
         call outfld('CFODD_NTOTAL15_CS', cfodd_ntotal15_cs, pcols, lchnk)
         ...
         call outfld('SLWC_COT11', slwc_cot11, pcols, lchnk)
         call outfld('SLWC_COT12', slwc_cot12, pcols, lchnk)
    endif    
    ...
    if (lmodis_sim) then
       ...
       ! CMB 04/19/24
       call outfld('CLMODIS_LWPR_MaCST',clmodis_lwpre_cam_macst  ,pcols,lchnk)
       call outfld('CLMODIS_LWPR_MaCAL',clmodis_lwpre_cam_macal  ,pcols,lchnk)
       call outfld('CLMODIS_LWPR_MoCAL',clmodis_lwpre_cam_mocal  ,pcols,lchnk)
  ...
  subroutine construct_cosp_outputs(Npoints,Ncolumns,Nlevels,Nlvgrid,Nchan,x)
    ...
    if (lmodis_sim) then
       ...
       ! CMB 04/19/24
       allocate(x%modis_LWP_vs_ReffLIQ_MaCST(nPoints,numMODISLWPBins,numMODISReffLiqBins))
       allocate(x%modis_LWP_vs_ReffLIQ_MaCAL(nPoints,numMODISLWPBins,numMODISReffLiqBins))
       allocate(x%modis_LWP_vs_ReffLIQ_MoCAL(nPoints,numMODISLWPBins,numMODISReffLiqBins))
...
allocate(x%modisandcloudsat_icecf(Npoints))
  ...
  subroutine destroy_cosp_outputs(y)
     ...
     ! CMB 04/19/24
     if (associated(y%modis_LWP_vs_ReffLIQ_MaCST))                   then
        deallocate(y%modis_LWP_vs_ReffLIQ_MaCST)
        nullify(y%modis_LWP_vs_ReffLIQ_MaCST)
     endif
     if (associated(y%modis_LWP_vs_ReffLIQ_MaCAL))                   then
        deallocate(y%modis_LWP_vs_ReffLIQ_MaCAL)
        nullify(y%modis_LWP_vs_ReffLIQ_MaCAL)
     endif
     if (associated(y%modis_LWP_vs_ReffLIQ_MoCAL))                   then
        deallocate(y%modis_LWP_vs_ReffLIQ_MoCAL)
        nullify(y%modis_LWP_vs_ReffLIQ_MoCAL)
     endif
